...
void CSimon::Render()
{
	...
	if (y < GROUND_Y)
	{
		if (isAttacking)
		{
			aniId = (direction >= 0) ? ID_ANI_SIMON_SIT_ATTACK_RIGHT : ID_ANI_SIMON_SIT_ATTACK_LEFT;
		}
		else
		{
			aniId = (direction >= 0) ? ID_ANI_SIMON_SIT_RIGHT : ID_ANI_SIMON_SIT_LEFT;
		}
	}
	else if (isSitting)
	{
	...
	else
	{
		if (isAttacking)
		{
			aniId = (direction > 0) ? ID_ANI_SIMON_STAND_ATTACK_RIGHT : ID_ANI_SIMON_STAND_ATTACK_LEFT;
		}
		else if (vx > 0)
		{
		...
		{
			aniId = (direction > 0) ? ID_ANI_SIMON_IDLE_RIGHT : ID_ANI_SIMON_IDLE_LEFT;
		}
	}
	currentId = aniId;
